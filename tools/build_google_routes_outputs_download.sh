...
FNR==NR {
  if (NR==1) next
  o=$1; d=$2; miles=$3; status=$5
  gsub(/^"|"$/, "", o)
  gsub(/^"|"$/, "", d)
  gsub(/^"|"$/, "", miles)
  gsub(/^"|"$/, "", status)
  if (status=="OK" && miles!="") dist[o SUBSEP d]=miles
  next
}
BEGIN {
  FPAT = "([^,]+)|("([^"]|"")+")"
...
'     "$dist_id" "$scenario_id" "google_routes_api_cached_od" "food_sctg_01_08" "truck" "triangular_fit"     "$p05" "$p50" "$p95" "$mean_m" "$min_m" "$max_m" "$n_records" "OK" "google_routes_api_cached_od" "$(csv_escape "$notes")" >> "$out_dist_csv"
done

pairs_requested="$(awk -F, 'NR>1 {n++} END{print n+0}' "$cache_csv")"
pairs_ok="$(awk -F, 'NR>1 && $5=="OK" {n++} END{print n+0}' "$cache_csv")"
pairs_error="$(awk -F, 'NR>1 && $5!="OK" {n++} END{print n+0}' "$cache_csv")"

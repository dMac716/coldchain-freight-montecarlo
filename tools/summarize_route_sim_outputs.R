...
for (f in track_files) {
  ...
  rows[[length(rows) + 1]] <- data.frame(
    ...
    truckloads_per_1e6_kcal = if (nrow(bsum) > 0 && "truckloads_per_1e6_kcal" %in% names(bsum)) as.numeric(bsum$truckloads_per_1e6_kcal[[1]]) else NA_real_,
    truckloads_per_1000kg_product = if (nrow(bsum) > 0 && "truckloads_per_1000kg_product" %in% names(bsum)) as.numeric(bsum$truckloads_per_1000kg_product[[1]]) else NA_real_,
    limiting_constraint = if (nrow(bsum) > 0 && "limiting_constraint" %in% names(bsum)) as.character(bsum$limiting_constraint[[1]]) else NA_character_,
    payload_utilization_pct = if (nrow(bsum) > 0 && "payload_utilization_pct" %in% names(bsum)) as.numeric(bsum$payload_utilization_pct[[1]]) else NA_real_,
    cube_utilization_pct = if (nrow(bsum) > 0 && "cube_utilization_pct" %in% names(bsum)) as.numeric(bsum$cube_utilization_pct[[1]]) else NA_real_,
    sanity_flag_low_cube_util = if (nrow(bsum) > 0 && "sanity_flag_low_cube_util" %in% names(bsum)) as.integer(bsum$sanity_flag_low_cube_util[[1]]) else NA_integer_,
    sanity_flag_high_cube_util = if (nrow(bsum) > 0 && "sanity_flag_high_cube_util" %in% names(bsum)) as.integer(bsum$sanity_flag_high_cube_util[[1]]) else NA_integer_,
    sanity_flag_unrealistic_case_dims = if (nrow(bsum) > 0 && "sanity_flag_unrealistic_case_dims" %in% names(bsum)) as.integer(bsum$sanity_flag_unrealistic_case_dims[[1]]) else NA_integer_,
...
out <- lapply(groups, function(d) {
  ...
  data.frame(
    ...
    p05_trucker_hours_per_1000kcal = as.numeric(stats::quantile(d$trucker_hours_per_1000kcal, 0.05, na.rm = TRUE, names = FALSE)),
    p50_trucker_hours_per_1000kcal = as.numeric(stats::quantile(d$trucker_hours_per_1000kcal, 0.50, na.rm = TRUE, names = FALSE)),
    p95_trucker_hours_per_1000kcal = as.numeric(stats::quantile(d$trucker_hours_per_1000kcal, 0.95, na.rm = TRUE, names = FALSE)),
    p05_truckloads_per_1e6_kcal = as.numeric(stats::quantile(d$truckloads_per_1e6_kcal, 0.05, na.rm = TRUE, names = FALSE)),
    p50_truckloads_per_1e6_kcal = as.numeric(stats::quantile(d$truckloads_per_1e6_kcal, 0.50, na.rm = TRUE, names = FALSE)),
    p95_truckloads_per_1e6_kcal = as.numeric(stats::quantile(d$truckloads_per_1e6_kcal, 0.95, na.rm = TRUE, names = FALSE)),
    p05_truckloads_per_1000kg_product = as.numeric(stats::quantile(d$truckloads_per_1000kg_product, 0.05, na.rm = TRUE, names = FALSE)),
    p50_truckloads_per_1000kg_product = as.numeric(stats::quantile(d$truckloads_per_1000kg_product, 0.50, na.rm = TRUE, names = FALSE)),
    p95_truckloads_per_1000kg_product = as.numeric(stats::quantile(d$truckloads_per_1000kg_product, 0.95, na.rm = TRUE, names = FALSE)),

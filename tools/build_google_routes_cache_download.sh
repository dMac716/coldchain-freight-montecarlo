...
'       "$origin_id" "$dest_id" "ERROR" "$(csv_escape "Missing zone centroid")" "$ts" "google_routes_v2_shell" >> "$out_cache_csv"
  ...
  body="$(jq -cn     --arg olat "$origin_lat"     --arg olon "$origin_lon"     --arg dlat "$dest_lat"     --arg dlon "$dest_lon"     '{
  ...
  if [[ "$first_logged" -eq 0 ]]; then
    ...
    printf '[debug] exec: '
    printf '%q '       curl -sS -X POST       "https://routes.googleapis.com/directions/v2:computeRoutes"       -H "Authorization: Bearer $safe_token"       -H "X-Goog-User-Project: $user_project"       -H "X-Goog-Api-Key: $safe_key"       -H "Content-Type: application/json"       -H "X-Goog-FieldMask: routes.distanceMeters,routes.duration"       --data "$body"
    printf '
'
    first_logged=1
  fi

...
if (!file.exists(opt$bev_plans_csv)) {
  stop(
    "BEV plans CSV not found: ", opt$bev_plans_csv,
    ". Generate it with: Rscript tools/route_precompute_bev_with_charging_google.R --routes ",
    opt$routes_csv, " --stations data/derived/ev_charging_stations_corridor.csv --output ", opt$bev_plans_csv
  )
}
...
plan_rows <- plans[, .(
  ...
  # Duration integrity: flag rows where traffic-adjusted duration is missing or
  # zero, which indicates the route cache used TRAFFIC_UNAWARE or had a bad row.
  duration_ok = all(
    is.finite(as.numeric(total_duration_s)) & as.numeric(total_duration_s) > 0
  ),
  # Provenance: record which routing_preference was used so we can flag mixing.
  routing_preference_used = if ("routing_preference" %in% names(plans)) {
    paste(sort(unique(as.character(routing_preference[nzchar(as.character(routing_preference))]))), collapse = "|")
  } else {
    NA_character_
  }
), by = .(route_id = as.character(route_id))]
...
# A plan fails if:
#   (a) it is long-haul with no BEV plan and no fallback allowed, OR
#   (b) its duration data is missing/zero (indicates a bad cache row or wrong routing_preference)
audit[, validation_status := ifelse(
  (is_longhaul & !plan_exists & !allow_fallback),
  "FAIL",
  ifelse(
    plan_exists & isFALSE(duration_ok),
    "WARN_DURATION",
    "PASS"
  )
)]

# Flag plans that were built from TRAFFIC_UNAWARE routes — these should not be
# treated as traffic-aware for scheduling or charging stop selection.
audit[, routing_preference_warning := ifelse(
  plan_exists &
    !is.na(routing_preference_used) &
    nzchar(routing_preference_used) &
    !grepl("TRAFFIC_AWARE", routing_preference_used),
  paste0("Non-traffic-aware routing used: ", routing_preference_used),
  NA_character_
)]
...
audit[, validation_message := ifelse(
  ...
  ifelse(
    validation_status == "WARN_DURATION",
    "BEV plan has missing or zero total_duration_s — check route cache routing_preference and distanceMeters presence",
    "OK"
  )
)]

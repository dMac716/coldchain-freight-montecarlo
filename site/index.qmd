...
`Latest production audit: audit_2026-03-17 | 75,443 runs | TRAFFIC_AWARE_OPTIMAL routing`
...
<div class="kpi-value">75,443</div>
Deduplicated, cross-platform validated
:::
:::
...
::: {.panel}
The production audit aggregated **75,443 validated runs** (62,565 diesel + 12,878 post-fix BEV) across 8 scenarios from 4 GCP and 4 Azure workers. Key findings:

- **Refrigerated**: BEV CO2/1000kcal (0.047) is comparable to diesel (0.048) despite 2.8x longer route (1,743 vs 617 mi) --- electrification offsets the distance penalty
- **Dry**: BEV CO2/1000kcal (0.028) comparable to diesel (0.022) --- BEV grid charging overhead nearly balanced by zero tailpipe emissions
- **BEV trip energy**: 4,000--4,050 kWh propulsion, 15--17 charging stops per 1,743-mi corridor
- **Reefer metrics**: diesel TRU uses 11.2 gal/trip; BEV electric TRU uses 439 kWh/trip
...
flowchart TD
  subgraph inputs["Data Inputs"]
    A1[data/inputs_local/*.csv<br/>scenarios, factors, priors]
    A2[data/derived/*.csv<br/>routes, BEV plans, OD cache]
  end

  subgraph sim["Simulation"]
    B[R/01_validate.R<br/>schema + mode gates]
    C[R/sim/01-10<br/>route segments, traffic,<br/>TRU, powertrain, charging]
    D[tools/run_route_sim_mc.R<br/>paired MC draws]
  end

  subgraph compute["Multi-Cloud Fleet"]
    E1[Local macOS]
    E2[GCP VMs x4]
    E3[Azure VMs x4]
  end
  ...
  subgraph collect["Collection & Analysis"]
    F[tools/pull_worker_results.sh<br/>GCS + Azure + local]
    G[tools/merge_run_bundles.sh<br/>dedup by run_id]
    H[tools/audit_analysis.R<br/>stats, figures, validation]
  end

  subgraph publish["Publishing"]
    I[artifacts/analysis_final_*/<br/>figures, tables, dataset]
    J[tools/update_site_from_audit.sh<br/>site assets + render]
    K[docs/ GitHub Pages]
  end
